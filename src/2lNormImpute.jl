...
function twoLevelNormImpute!(
    ...
    for _ in 1:nIterations
        # Draw random class-level coefficients and within-class sums of squares
        for class in 1:nClasses
            ...
            noise = cholesky(symridge(βVar, ridge)).U' * randn(nRc)
            β[class, :] = meanPart + noise
        ...
        ψScale = chol2inv(cholesky(symridge(transpose(centred) * centred, ridge)))
        invΨ = randWishart(nClasses - nRc - 1, cholesky(ψScale).U)

        # Draw class-specific precisions
        for class in 1:nClasses
